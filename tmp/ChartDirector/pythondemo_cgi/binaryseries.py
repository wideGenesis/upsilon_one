...
dataX = [chartTime(2008, 7, 1, 0, 0, 0), chartTime(2008, 7, 1, 2, 17, 2), chartTime(2008, 7, 1, 8,
    5, 30), chartTime(2008, 7, 1, 10, 54, 10), chartTime(2008, 7, 1, 15, 40, 0), chartTime(2008, 7,
    1, 18, 22, 20), chartTime(2008, 7, 1, 22, 17, 14), chartTime(2008, 7, 2, 2, 55, 50), chartTime(
    2008, 7, 2, 8, 17, 14), chartTime(2008, 7, 2, 11, 55, 50), chartTime(2008, 7, 2, 13, 17, 14),
    chartTime(2008, 7, 2, 17, 55, 50), chartTime(2008, 7, 2, 20, 17, 14), chartTime(2008, 7, 3, 0,
    0, 0)]
...
# all other labels.
c.xAxis().setMultiFormat(StartOfDayFilter(), "<*font=arialbd.ttf*>{value|mmm dd}", AllPassFilter(),
    "{value|hh:nn}")
...
shiftedLine0 = ArrayMath(dataY).add(4.5).result()
...
shiftedLine1 = ArrayMath(dataY).add(2.5).result()
# Reverse the data, so the 0 becomes 1 and 1 becomes 0, and shift it as well.
reverseShiftedLine1 = ArrayMath(dataY).mul(-1).add(3.5).result()
...
shiftedLine2 = ArrayMath(dataY).add(0.5).result()
# Reverse the data, so the 0 becomes 1 and 1 becomes 0, and shift it as well.
reverseShiftedLine2 = ArrayMath(dataY).mul(-1).add(1.5).result()
