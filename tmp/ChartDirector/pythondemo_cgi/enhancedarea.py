...
c = XYChart(600, 360, brushedSilverColor(), Transparent, 1)
c.setRoundedFrame()
c.setDropShadow()
...
contentBox.setSize(c.getDrawArea().getWidth() - 1 - frameWidth * 2, c.getDrawArea().getHeight(
...
plotArea = c.setPlotArea(50 + contentBox.getLeftX(), contentBox.getTopY() + 25, contentBox.getWidth(
    ) - 75, contentBox.getHeight() - 75, 0xffffff, -1, -1, 0xcccccc, -1)
...
c.addAreaLayer(data, c.linearGradientColor(0, contentBox.getTopY() + 20, 0, contentBox.getTopY(
...
c.addText(plotArea.getLeftX() + plotArea.getWidth() - 5, plotArea.getTopY() + plotArea.getHeight(
    ) - 5,
