...
def plot_histogram(returns, resample="M", bins=20,
    ...
    _sns.distplot(returns, bins=bins,
                  ...
                  kde_kws=dict(color='#ffffff', alpha=.7),
                  ax=ax)

    ax.xaxis.set_major_formatter(_plt.FuncFormatter(
        lambda x, loc: "{:,}%".format(int(x*100))))

    ax.axhline(0.01, lw=1, color="black", zorder=2)
    ax.axvline(0, lw=1, color="black", zorder=2)
...
def plot_longest_drawdowns(returns, periods=6, lw=1.5,
                           fontname='Arial', grayscale=False,
                           log_scale=False, figsize=(10, 6), ylabel=True,
                           subtitle=True, compounded=True,
                           savefig=None, show=True):

    colors = ['#348dc1', '#003366', 'lime']
    if grayscale:
        colors = ['#000000'] * 2
    ...
    ax.axhline(0, ls="--", lw=1, color="silver", zorder=2) #000000
    _plt.yscale("symlog" if log_scale else "linear")
    if ylabel:
...
def plot_table(tbl, columns=None, title="", title_loc="left",
    ...
    if title != "":
        ax.set_title(title, fontweight="bold",
                     fontsize=14, color="white", loc=title_loc)
    ...
    for (row, col), cell in the_table.get_celld().items():
        ...
        cell.set_edgecolor('black') # #dddddd
        if row == 0 and header:
