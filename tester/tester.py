...
def main():
    portfolio_tester(init_cap=10000, port_id='leveraged', allocator_data_interval=3, selector_data_interval=12,
                     start_test_date=datetime.date(2010, 1, 1))
    pass
...
if __name__ == '__main__':
    ...
    # p3.start()
    p4 = mp.Process(target=portfolio_tester, args=(10000, 'aggressive', 3, 12, datetime.date(2015, 11, 1),))
    p4.start()
    p5 = mp.Process(target=portfolio_tester, args=(10000, 'leveraged', 3, 12,  datetime.date(2015, 11, 1),))
    p5.start()
    # p1.join()
    # p2.join()
    # p3.join()
